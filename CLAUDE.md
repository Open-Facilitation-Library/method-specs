...
- **`branch-links` CI rejects a Harmonica ticket ID in any tracked file — path or content.** This registry is deliberately tracker-independent (`4516a79`), so `docs/conformance/reference-cases/har-1697.md` failed the check on both its filename and one sentence naming the ticket; renamed to `harmonica-chain-quality-gate.md`. Name a reference case for the implementation, and cite internal tickets from the PR or issue instead, which are not tracked files.
