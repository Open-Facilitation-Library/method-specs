...
for (const m of listMethodDirs(methodsRoot)) {
  ...
  if (typeof id === 'string') {
    ...
    specsMeta.push({ id, version: parsed.frontmatter.version, composes: parsed.frontmatter.composes });
  }
  if (errors.length) {
...
const comp = validateComposition(specsMeta);
comp.errors.forEach((e) => {
  console.error(`✗ composition: ${e}`);
  errorCount++;
});
comp.warnings.forEach((w) => console.warn(`    warn: ${w}`));
